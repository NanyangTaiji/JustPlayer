...
import com.nytaiji.nybase.utils.ProgressHandler;
...
public class EncryptUtil {
 ...
 /*   public static HybridFile ConstructEncryptTarget(HybridFile targetFile, String password) {
    ...
    public static CTRnoPadding LevelCipherPackage(String password) {
        ...
        mCTR.secretKeySpec = new SecretKeySpec(key, Constants.AES_ALGORITHM);
        mCTR.ivParameterSpec = new IvParameterSpec(iv);
        try {
            mCTR.cipher = Cipher.getInstance(Constants.CTR_TRANSFORMATION);
    ...
    public Cipher CTRNoppadingCipher(String KeyS, String ivS) {
        ...
        Cipher mCipher;
        SecretKeySpec mSecretKeySpec = new SecretKeySpec(key, Constants.AES_ALGORITHM);
        IvParameterSpec mIvParameterSpec = new IvParameterSpec(iv);
        try {
            mCipher = Cipher.getInstance(Constants.CTR_TRANSFORMATION);
    ...
    public static int encryptLevelFromFileName(String mUrl) {
        int tempI = originalLevelFromFileName(mUrl);
        return (tempI > 5) ? tempI - 5 : tempI;
    }

    public static int originalLevelFromFileName(String mUrl) {
        int tempI = -1;
    ...
    public static String getDecryptFileName(String path) {

        return path.replace("_NY", "_");
    }
    ...
    public static boolean defaultEncryptFile(Context context, File infile, String passWord) {
        ...
        String ext = NyFileUtil.getFileExtension(infilePath);
        String newName = NyFileUtil.getFileNameWithoutExtFromPath(infilePath) + "_NY0." + ext;
    ...
    public static boolean EncryptMergedFile(Context context, File infile, File outfile, Cipher
        ...
        try {
            ...
            if (mergeF != null && mergeF.exists() && mergeF.canRead() && mergeF.length() > 0) {
                ...
                byte[] arr = new byte[(int) Constants.ENCRYPT_SKIP];
                rd.nextBytes(arr);
    ...
    public static boolean DecryptMergedFile(Context context, File infile, File
        ...
        try {
            ...
            if (mergeF != null) {
                ...
                byte[] bufferM = new byte[(int) Constants.ENCRYPT_SKIP];
                cipherinputputStream.read(bufferM);
                fom.write(bufferM, 0, (int) Constants.ENCRYPT_SKIP);
                fom.close();
                Log.e("DecryptMergedFile", "head " + mergeF.length());
            } else {
                sourceStream.skip(Constants.ENCRYPT_SKIP);
                AesHelper.jumpToOffset(ces.cipher, ces.secretKeySpec, ces.ivParameterSpec, Constants.ENCRYPT_SKIP);
            }
    ...
    public static void registerEncryptedVideo(Context context, File outfile) {
        ...
        if (nyVideo != null) {
            ...
            NyFileUtil.insertNyVideo(context, nyVideo);
        }
    }
   ...
   /* public static boolean AdvancedEncryptFile(Context context, File infile, File outfile, String
    ...
    public static String StrToEncryptStr(String inStr, byte[] key, byte[] iv) {
        ...
        IvParameterSpec mIvParameterSpec = new IvParameterSpec(iv);
        try {
            Cipher mCipher = Cipher.getInstance(Constants.CTR_TRANSFORMATION);
            mCipher.init(Cipher.DECRYPT_MODE, mSecretKeySpec, mIvParameterSpec);
    ...
    public static String FileToEncryptStr(String path, String KeyS, String ivS) {
        String input = null;
        try {
            input = NyFileUtil.convertFileToString(path);
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//--------------------
    ...
    public static void AnnouncedAndRegister(Context context, File infile) {
        ...
        intent.putExtra(Constants.DOWNLOAD_URL, filePath);
        intent.putExtra(Constants.DOWNLOAD_FULL_NAME, fileName);
        // 发送广播
