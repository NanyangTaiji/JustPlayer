 ...
 */
public abstract class AmazeStreamServer {
  private static final String TAG = "AmazeStreamServer";
  ...
  public class Response {
    ...
    /** Basic constructor. */
    public Response(String status, String mimeType, AmazeStreamSource data) {
      this.status = status;
  ...
  public AmazeStreamServer(int port, File wwwroot) throws IOException {
    myTcpPort = port;
        ...
        new Thread(
            () -> {
              try {
                ...
                Log.e(TAG, "failed to start httpsession"+ioe.toString());
              }
            });
  ...
  public AmazeStreamServer(File wwwroot) throws IOException {
    myServerSocket = tryBind(myTcpPort);
    myThread =
        new Thread(
            () -> {
  ...
  public void stop() {
    try {
      ...
      Log.w(TAG, "failed to stop server "+ioe.toString());
    }
  }
  ...
  private class HTTPSession implements Runnable {
      ...
      try {
        ...
        if (is != null) {
          try {
            ...
            Log.w(TAG, "failed to close socket "+ e.toString());
          }
        }
      }
    }
    ...
    private void sendResponse(
        Socket socket, String status, String mime, Properties header, AmazeStreamSource data) {
      try {
