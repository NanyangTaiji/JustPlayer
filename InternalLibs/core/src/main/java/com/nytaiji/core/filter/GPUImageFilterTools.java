...
public class GPUImageFilterTools {
    ...
    public static void showDialog(final Context context,
        ...
        filters.addFilter("Exposure", ImageFilterType.EXPOSURE);
        filters.addFilter("Brightness", ImageFilterType.BRIGHTNESS);
        filters.addFilter("Contrast", ImageFilterType.CONTRAST);
        filters.addFilter("Sharpness", ImageFilterType.SHARPEN);
        filters.addFilter("Hue", ImageFilterType.HUE);
        filters.addFilter("Color Balance", ImageFilterType.COLOR_BALANCE);
        filters.addFilter("Gamma", ImageFilterType.GAMMA);
        filters.addFilter("Saturation", ImageFilterType.SATURATION);
        filters.addFilter("White Balance", ImageFilterType.WHITE_BALANCE);
        filters.addFilter("Highlight Shadow", ImageFilterType.HIGHLIGHT_SHADOW);
        filters.addFilter("Vignette", ImageFilterType.VIGNETTE);
        filters.addFilter("ToneCurve", ImageFilterType.TONE_CURVE);
        filters.addFilter("Opacity", ImageFilterType.OPACITY);
        filters.addFilter("Vibrance", ImageFilterType.VIBRANCE);
        filters.addFilter("Luminance", ImageFilterType.LUMINANCE);
        filters.addFilter("Haze", ImageFilterType.HAZE);
        filters.addFilter("Levels Min (Mid Adjust)", ImageFilterType.LEVELS_FILTER_MIN);
        filters.addFilter("Gaussian Blur", ImageFilterType.GAUSSIAN_BLUR);
        filters.addFilter("Box Blur", ImageFilterType.BOX_BLUR);
        filters.addFilter("Bilateral Blur", ImageFilterType.BILATERAL_BLUR);
        filters.addFilter("Transform (2-D)", ImageFilterType.TRANSFORM2D);
        filters.addFilter("Solarize", ImageFilterType.SOLARIZE);
        filters.addFilter("Monochrome", ImageFilterType.MONOCHROME);
        filters.addFilter("Pixelation", ImageFilterType.PIXELATION);
        filters.addFilter("Sepia", ImageFilterType.SEPIA);
        filters.addFilter("Grayscale", ImageFilterType.GRAYSCALE);
        filters.addFilter("3x3 Convolution", ImageFilterType.THREE_X_THREE_CONVOLUTION);
        filters.addFilter("Emboss", ImageFilterType.EMBOSS);
        filters.addFilter("Posterize", ImageFilterType.POSTERIZE);
        filters.addFilter("Invert", ImageFilterType.INVERT);
        filters.addFilter("Sobel Edge Detection", ImageFilterType.SOBEL_EDGE_DETECTION);

        filters.addFilter("Luminance Threshold", ImageFilterType.LUMINANCE_THRESHSOLD);
        filters.addFilter("Threshold Edge Detection", ImageFilterType.THRESHOLD_EDGE_DETECTION);
        filters.addFilter("Sketch", ImageFilterType.SKETCH);
        filters.addFilter("Laplacian", ImageFilterType.LAPLACIAN);
        filters.addFilter("Toon", ImageFilterType.TOON);
        filters.addFilter("Smooth Toon", ImageFilterType.SMOOTH_TOON);
        filters.addFilter("Halftone", ImageFilterType.HALFTONE);
        filters.addFilter("Crosshatch", ImageFilterType.CROSSHATCH);
        filters.addFilter("Weak Pixel Inclusion", ImageFilterType.WEAK_PIXEL_INCLUSION);
        filters.addFilter("False Color", ImageFilterType.FALSE_COLOR);
        filters.addFilter("Lookup (Amatorka)", ImageFilterType.LOOKUP_AMATORKA);
        filters.addFilter("Grouped filters", ImageFilterType.FILTER_GROUP);
        filters.addFilter("RGB", ImageFilterType.RGB);
        filters.addFilter("CGA Color Space", ImageFilterType.CGA_COLORSPACE);
        filters.addFilter("Dilation", ImageFilterType.DILATION);
        filters.addFilter("Kuwahara", ImageFilterType.KUWAHARA);
        filters.addFilter("RGB Dilation", ImageFilterType.RGB_DILATION);
        filters.addFilter("Non Maximum Suppression", ImageFilterType.NON_MAXIMUM_SUPPRESSION);

        filters.addFilter("Bulge Distortion", ImageFilterType.BULGE_DISTORTION);
        filters.addFilter("Glass Sphere", ImageFilterType.GLASS_SPHERE);
        filters.addFilter("Sphere Refraction", ImageFilterType.SPHERE_REFRACTION);
        filters.addFilter("Swirl", ImageFilterType.SWIRL);
        
        filters.addFilter("Blend (Difference)", ImageFilterType.BLEND_DIFFERENCE);
        filters.addFilter("Blend (Source Over)", ImageFilterType.BLEND_SOURCE_OVER);
        filters.addFilter("Blend (Color Burn)", ImageFilterType.BLEND_COLOR_BURN);
        filters.addFilter("Blend (Color Dodge)", ImageFilterType.BLEND_COLOR_DODGE);
        filters.addFilter("Blend (Darken)", ImageFilterType.BLEND_DARKEN);
        filters.addFilter("Blend (Dissolve)", ImageFilterType.BLEND_DISSOLVE);
        filters.addFilter("Blend (Exclusion)", ImageFilterType.BLEND_EXCLUSION);
        filters.addFilter("Blend (Hard Light)", ImageFilterType.BLEND_HARD_LIGHT);
        filters.addFilter("Blend (Lighten)", ImageFilterType.BLEND_LIGHTEN);
        filters.addFilter("Blend (Add)", ImageFilterType.BLEND_ADD);
        filters.addFilter("Blend (Divide)", ImageFilterType.BLEND_DIVIDE);
        filters.addFilter("Blend (Multiply)", ImageFilterType.BLEND_MULTIPLY);
        filters.addFilter("Blend (Overlay)", ImageFilterType.BLEND_OVERLAY);
        filters.addFilter("Blend (Screen)", ImageFilterType.BLEND_SCREEN);
        filters.addFilter("Blend (Alpha)", ImageFilterType.BLEND_ALPHA);
        filters.addFilter("Blend (Color)", ImageFilterType.BLEND_COLOR);
        filters.addFilter("Blend (Hue)", ImageFilterType.BLEND_HUE);
        filters.addFilter("Blend (Saturation)", ImageFilterType.BLEND_SATURATION);
        filters.addFilter("Blend (Luminosity)", ImageFilterType.BLEND_LUMINOSITY);
        filters.addFilter("Blend (Linear Burn)", ImageFilterType.BLEND_LINEAR_BURN);
        filters.addFilter("Blend (Soft Light)", ImageFilterType.BLEND_SOFT_LIGHT);
        filters.addFilter("Blend (Subtract)", ImageFilterType.BLEND_SUBTRACT);
        filters.addFilter("Blend (Chroma Key)", ImageFilterType.BLEND_CHROMA_KEY);
        filters.addFilter("Blend (Normal)", ImageFilterType.BLEND_NORMAL);
    ...
    public static GPUImageFilter createFilterForType(final Context context, final ImageFilterType type) {
        switch (type) {
            case CONTRAST:
            ...
            default:
                Log.e("GPUImageFilterTools","No filter of  "+type.name());
                return null;
    ...
    public static GPUImageFilter createBlendFilter(Context context, Class<? extends GPUImageTwoInputFilter> filterClass) {
        try {
            ...
            filter.setBitmap(BitmapFactory.decodeResource(context.getResources(), com.nytaiji.nybase.R.drawable.banner));
            return filter;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    public enum ImageFilterType {
        DEFAULT,CONTRAST, GRAYSCALE, SHARPEN, SEPIA, SOBEL_EDGE_DETECTION, THRESHOLD_EDGE_DETECTION, THREE_X_THREE_CONVOLUTION, FILTER_GROUP, EMBOSS, POSTERIZE, GAMMA, BRIGHTNESS, INVERT, HUE, PIXELATION,
    ...
    private static class FilterList {
        ...
        public List<ImageFilterType> filters = new LinkedList<ImageFilterType>();

        public void addFilter(final String name, final ImageFilterType filter) {
            names.add(name);
            filters.add(filter);
        }
    }
    ...
    public static class FilterAdjuster {
        ...
        public FilterAdjuster(final GPUImageFilter filter) {
            if (filter instanceof GPUImageSharpenFilter) {
                ...
                adjuster = new LuminanceThresholdAdjuster().filter(filter);
         ...
         //   } else if (filter instanceof GPUImageSwirlFilter) {
          //      adjuster = new SwirlAdjuster().filter(filter);
                ...
                adjuster = new VibranceAdjuster().filter(filter);
            } else {
                adjuster = null;
            }
        }
        ...
        private class ContrastAdjuster extends Adjuster<GPUImageContrastFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setContrast(range(percentage, 0.1f, 3.0f));
            }
        }
        ...
        private class GammaAdjuster extends Adjuster<GPUImageGammaFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setGamma(range(percentage, 0.1f, 3.0f));
            }
        }
        ...
        private class LuminanceThresholdAdjuster extends Adjuster<GPUImageLuminanceThresholdFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setThreshold(range(percentage, 0.05f, 0.95f));
            }
        }
        ...
        private class CrosshatchBlurAdjuster extends Adjuster<GPUImageCrosshatchFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setCrossHatchSpacing(range(percentage, 0.005f, 0.04f));
                getFilter().setLineWidth(range(percentage, 0.005f, 0.006f));
            }
        }
