...
import static com.nytaiji.nybase.utils.BitmapUtil.decodeResource;
import static com.nytaiji.nybase.utils.BitmapUtil.toBase64;
import static com.nytaiji.nybase.utils.BitmapUtil.toBitmap;
...
//------------------------------------------------------------------------------//
    ...
    private static final String BASE_HTML = "file:///android_asset/editor.html";
    ...
    public void initView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        addJavascriptInterface(new JavascriptInterfaceVideoOnclick(), "videoOnclick");
        //--------------------//
        loadUrl(BASE_HTML);
    ...
    loadScript("javascript:RE.prepareInsert();");
    loadScript("javascript:RE.insertImageW('" + url + "', '" + alt + "','" + width + "');");
    }
    ...
    private String convertHexColorString(int color) {
        //  Log.e("RichEditor","color= "+color);
        //  Log.e("RichEditor","convertHexColorString= "+String.format("#%06X", (0xFFFFFF & color)));
    ...
    public OnClickVideoListener mOnClickVideoListener;

    protected void exec(final String script) {
        if (isReady) {
            loadScript(script);
        } else {
    ...
    public class JavascriptInterfaceImageOnclick {
        ...
        public void openImage(String imgUrl) {
            if (mOnClickImageListener != null && !TextUtils.isEmpty(imgUrl)) {
                mOnClickImageListener.onClick(imgUrl);
            }
        }
    }

    public class JavascriptInterfaceVideoOnclick {

        @android.webkit.JavascriptInterface
        public void openVideo(String videoUrl) {
            if (mOnClickVideoListener != null && !TextUtils.isEmpty(videoUrl)) {
                mOnClickVideoListener.onClick(videoUrl);
            }
        }
    }
}
