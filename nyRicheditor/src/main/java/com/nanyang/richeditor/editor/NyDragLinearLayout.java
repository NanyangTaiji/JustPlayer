...
public class NyDragLinearLayout extends LinearLayout {
    private static final String TAG = NyDragLinearLayout.class.getSimpleName();
    ...
    public NyDragLinearLayout(Context context) {
        this(context, null);
    }

    public NyDragLinearLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void setViewDraggable(View child, View dragHandle) {
        if (null == child) {
            throw new IllegalArgumentException(
                    "Draggable children must not be null.");
        }

        if (this == child.getParent()) {
            if (dragHandle != null)
                dragHandle.setOnTouchListener(new DragHandleOnTouchListener(child));
            child.setOnLongClickListener(mLongClickDragListener);
    ...
    private void startDelete() {
        final int i = mDragItem.mPosition;
        //  Log.e(TAG, "start delete i= "+i);
        if (i >= 0) {
            AlertDialog.Builder builder = new AlertDialog.Builder(getContext());
            if (getChildAt(i) instanceof TextView)
            builder.setMessage("Remove item " + ((TextView) getChildAt(i)).getText().toString() + "?");
            else builder.setMessage("Remove the selected item? " + ((TextView) getChildAt(i)).getText().toString() + "?");
