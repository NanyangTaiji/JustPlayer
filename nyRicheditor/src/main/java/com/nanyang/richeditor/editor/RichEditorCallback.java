...
public abstract class RichEditorCallback {
    ...
    private final List<ActionType> mFontBlockGroup =
        Arrays.asList(ActionType.NORMAL, ActionType.H1, ActionType.H2, ActionType.H3, ActionType.H4, ActionType.H5, ActionType.H6);
    private final List<ActionType> mTextAlignGroup =
        Arrays.asList(ActionType.JUSTIFY_LEFT, ActionType.JUSTIFY_CENTER, ActionType.JUSTIFY_RIGHT, ActionType.JUSTIFY_FULL);
    private final List<ActionType> mListStyleGroup =
    ...
    private void updateStyle(FontStyle fontStyle) {
        if (mFontStyle.getFontFamily() == null || !mFontStyle.getFontFamily().equals(fontStyle.getFontFamily())) {
        ...
        if (mFontStyle.getFontForeColor() == null || !mFontStyle.getFontForeColor().equals(fontStyle.getFontForeColor())) {
            if (!TextUtils.isEmpty(fontStyle.getFontForeColor())) {
                notifyFontStyleChange(ActionType.FORE_COLOR, fontStyle.getFontForeColor());
            }
        }

        if (mFontStyle.getFontBackColor() == null || !mFontStyle.getFontBackColor().equals(fontStyle.getFontBackColor())) {
